...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for the generate-then-evaluate workflow."""
...
def main() -> None:
    """CLI entrypoint for dataset generation + benchmark evaluation."""
    parser = build_parser()
    ...
    if not args.skip_generation and Path(args.report_dir) != Path(DEFAULT_REPORT_DIR):
        parser.error(
            ...
            f"because dataset generation currently writes to {DEFAULT_REPORT_DIR}."
        )
...
__all__ = ["DEFAULT_REPORT_DIR", "build_parser", "main"]
