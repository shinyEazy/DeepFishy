"""Prompt builders and constants for benchmark evaluation."""

import base64

from langchain_core.messages import HumanMessage, SystemMessage

TOPIC = "CTCP Tap doan Hoa Phat (HPG) trong quy 3 nam 2025"
...
def _pdf_content_blocks(pdf_bytes: bytes, label: str) -> list[dict]:
    ...
    try:
        for page_index, page in enumerate(pdf, start=1):
            ...
            blocks.append({"type": "text", "text": f"{label} - page {page_index}"})
            blocks.append(
                {
                    "type": "image_url",
...
def format_evaluation_prompt(
    ...
    content_blocks.extend(_pdf_content_blocks(golden_pdf, "GOLDEN STANDARD REPORT"))
