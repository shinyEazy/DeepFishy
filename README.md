...
Generate report using given topic

```bash
uv run python -m deepfishy.features.reports.application.generate_report \
  --topic "Công ty Cổ phần Sữa Việt Nam (Vinamilk – VNM) trong giai đoạn 2025–2026"
```
...
To evaluate a generated report with a reference report

```bash
uv run python -m deepfishy.features.benchmark.evaluator \
  --topic "Ngân hàng TMCP Quân đội (MBBank – MBB) trong giai đoạn 2025–2026" \
  --generated_report benchmark/generated_reports/open_deep_research/topic_1.pdf \
  --golden_report benchmark/golden_reports/mbb.pdf
```
